...
class DisasterScene3D {
  ...
  setRouteHighlight(mode) {
    ...
    if (mode === "safest") {
      this.safestRouteMesh.visible = true;
      this.moderateRouteMesh.visible = false;
      this.hardRouteMesh.visible = false;
    } else if (mode === "quickest" || mode === "moderate") {
      this.safestRouteMesh.visible = false;
      this.moderateRouteMesh.visible = true;
      this.hardRouteMesh.visible = false;
    } else if (mode === "hard" || mode === "shortest") {
      this.safestRouteMesh.visible = false;
      this.moderateRouteMesh.visible = false;
      this.hardRouteMesh.visible = true;
    } else {
      this.safestRouteMesh.visible = true;
      this.moderateRouteMesh.visible = true;
      this.hardRouteMesh.visible = true;
  ...
  displayMultiVictimTour(tour) {
    if (this.multiTourMesh) this.routesGroup.remove(this.multiTourMesh);
    if (!tour || !tour.sequence) return;
    const start = window.RESCUE_CONFIG.groundTeam.currentLocation;
    const pts = [new THREE.Vector3(start.x, 0.8, start.z)];
    tour.sequence.forEach(s => {
      const v = s.victim;
      if (v && v.surfaceCoords) {
        pts.push(new THREE.Vector3(v.surfaceCoords.x, 0.8, v.surfaceCoords.z));
      }
    });
    if (pts.length > 1) {
      const curve = new THREE.CatmullRomCurve3(pts);
      const geo = new THREE.TubeGeometry(curve, 80, 0.4, 8, false);
      const mat = new THREE.MeshStandardMaterial({ color: 0x8b5cf6, emissive: 0x6d28d9, emissiveIntensity: 0.9 });
      this.multiTourMesh = new THREE.Mesh(geo, mat);
      this.routesGroup.add(this.multiTourMesh);
    }
  }
